...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    error ZkMinimalAccount__ExecuteCallFailed();
     ...
     * @notice check to see if we have enough balance to pay for the transaction
     */
    ...
    function _validateTransaction(
        ...
        bool isVaildSigner = signer == owner();
        if (isVaildSigner) {
    ...
    function _executeTransaction(Transaction memory _transaction) internal {
        ...
        if (to == address(DEPLOYER_SYSTEM_CONTRACT)) {
            ...
            if (!success) {
                revert ZkMinimalAccount__ExecuteCallFailed();
            }
        }
    }
}
